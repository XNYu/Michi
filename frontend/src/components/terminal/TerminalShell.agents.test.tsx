...
vi.mock('../../state/chatStore', async () => {
  ...
  return { ...actual, useChatProjects: () => ({ activeProject: project(), activeProjectId: shell.activeProjectId, projects: [project()], order: [], edges: [], theme: 'light', availableModes: [], agentStatus: null, warmFailedError: null, openPanes: [], focusedPane: null, focusedNodeId: null, viewMode: 'single', selection: new Set(), hydrated: true, treeSelection: new Set(), searchHighlightTerm: null, canNavBack: false, canNavForward: false }), useChatActions: () => ({ createProject: async () => 'p', enterChatsWorkspace: async () => 'p', focusPane: () => {}, closePane: () => {}, openPane: () => {}, createBlankChild: async () => {}, restoreLastDeletion: () => null, clearSelection: () => {}, clearTreeSelection: () => {}, selectAllTrees: () => {}, navBack: () => {}, navForward: () => {} }), useStructuralSelector: (selector: (nodes: Record<string, never>) => unknown) => selector({}), useNodesSelector: () => 0, chatLabel: () => '' };
});
